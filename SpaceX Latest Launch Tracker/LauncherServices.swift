...
public class LauncherServices {
    static func fetchLatestLauncher(completion: @escaping (Result<Modals.LauncherResponse, Error>) -> Void) {
        let url = "https://api.spacexdata.com/v4/launches/latest"
        
        AF.request(url).responseDecodable(of: Modals.LauncherResponse.self) { response in
